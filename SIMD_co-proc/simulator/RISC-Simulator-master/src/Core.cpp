...
void Core::decode() {
	...
	bool temp_isMov;
	//n
	bool temp_isVmov1 = false;
	bool temp_isVmov2 = false;
	bool temp_isVadd = false;
	bool temp_isVsub = false;
	bool temp_isVmul = false;
	bool temp_isVdiv = false;
	bool temp_isVmod = false;
	bool temp_isVand = false;
	bool temp_isVld = false;
	bool temp_isVst = false;
	bool temp_isVwb;
	//n
	...
	if(  !(opcode5 == 1 || ( opcode5 == 0 && opcode3 == 1 && opcode1 == 1 &&  (opcode4 == 1||opcode2==0) ))||(opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0)	|| 	(opcode5 == 1 && opcode4 == 0 && opcode3 == 0 && opcode2 == 1 && opcode1 == 1)   ){
		temp_isWb = true;

		//cout<<opcode5<<opcode4<<opcode3<<opcode2<<opcode1<<endl;
		//pprint(2)<<"isWb ";
	}
	else{
		temp_isWb = false;
	...
	if((opcode5==1 && opcode4==1 && opcode3==0) ||(opcode5==1 && opcode4==1 && opcode3==1 && opcode2==0)||(opcode5==1 && opcode4==0&&opcode3==1&&((opcode2!=0&&opcode1!=0)||(opcode2!=1 && opcode1!=0)) )){
		temp_isVwb=true;
	}
	else{
		temp_isVwb=false;
	...
	//n
	unsigned long long int temp_VA = 0;
	unsigned long long int temp_VB;
	//!n
	...
	//n
	unsigned long long int temp_voperand1;
	unsigned long long int temp_voperand2;
	...
	//n
	bool sig = (opcode5 & (opcode4 | (opcode3 & opcode2)));  //toc heck whether instruction opcode is greater than or equal to 22 or not
	//!n
	if(!sig){
		if (temp_isRet){
			temp_operand1 = R[15];
		...
		else if(temp_isVmov1){
			temp_operand1 = R[temp_rs2];
		...
		if (temp_isSt){
			...
			temp_operand2 = R[(temp_rs2+1)%16];
			//fprint(1)<<"; "<<registerstring((temp_rs2+1)%16)<<" = 0x";
		}
		else{
		...
		if(temp_isVst){
			temp_voperand2 = V[temp_rd];
			temp_A = R[temp_rs1];
			fprint(1)<<"; "<<registerstring(temp_rs1)<<" = 0x";
			fprint(1)<<hex<<R[temp_rs1];
			//fprint(1)<<"; "<<vectorstring(temp_rd)<<" = 0x";
		}
		else if(temp_isVld){
			temp_voperand2 = V[temp_rs2];
			temp_A = R[temp_rs1];
			fprint(1)<<"; "<<registerstring(temp_rs1)<<" = 0x";
			fprint(1)<<hex<<R[temp_rs1];
		}
		else if(temp_isVmov2){
			temp_voperand1 = V[temp_rs2];
			fprint(1)<<"; "<<registerstring(temp_rs2)<<" = 0x";
		}
		else{
			temp_voperand1 = V[temp_rs1];
			//cout << temp_rs1 << endl;
			fprint(1)<<"; "<<vectorstring(temp_rs1)<<" = 0x";
			fprint(1)<<hex<<temp_voperand1;
			temp_voperand2 = V[temp_rs2];
			if (!temp_isImmediate){
		...
		temp_VA = temp_voperand1;
		if (!temp_isImmediate){
			
			fprint(1)<<hex<<temp_voperand2;
		}
		else{
			fprint(1)<<"; imm = 0x"<<hex<<temp_immx;
		}
	...
	if (temp_isImmediate){
		temp_B = temp_immx;
		if(!(temp_isVst || temp_isVld))
			temp_VB = (temp_immx)|((unsigned long long int)temp_immx<<16)|((unsigned long long int)temp_immx<<32)|((unsigned long long int)temp_immx<<48);
	...
	//n
	of_ex.VA.Write(temp_VA);
	of_ex.VB.Write(temp_VB);
	of_ex.Voperand2.Write(temp_voperand2);
...
void Core::execute() {
	...
	unsigned long long int temp_VA=of_ex.VA.Read();
	unsigned long long int temp_VB=of_ex.VB.Read();
	unsigned long long int temp_voperand2=of_ex.Voperand2.Read();
	bool temp_isVwb=of_ex.isVwb.Read();
	...
	//n
	unsigned long long int temp_valuresult=0;
	...
	if(temp_isVadd){
			...
			unsigned long long int lb;
			a=temp_VA+temp_VB;
			temp_valuresult |=(a);
			la=(temp_VA<<32)>>48;
			lb=(temp_VB<<32)>>48;
			a=la+lb;
			temp_valuresult|=(a<<16);
			la=(temp_VA<<16)>>48;
			lb=(temp_VB<<16)>>48;
			a=lb+la;
			temp_valuresult|=((unsigned long long int)a<<32);
			la=(temp_VA)>>48;
			lb=(temp_VB)>>48;
			a=la+lb;
			temp_valuresult|=((unsigned long long int)a<<48);
	}
	if(temp_isVsub){
			unsigned short int a;
			unsigned long long int la;
	...
	if(temp_isVand){
			temp_valuresult=(temp_VA & temp_VB);
	}
	if(temp_isVmov1){
		//cout << "asdas "<<temp_A<<endl;
		//cout << "asdas "<<temp_B<<endl;
		temp_valuresult=((unsigned long long int)temp_A<<32|temp_B);
		//cout << hex<<temp_valuresult << endl;
	}
	if(temp_isVmov2){
		temp_valuresult=temp_VA;
	}
	//n
	if(temp_isVld){
	...
	ex_ma.Voperand2.Write(temp_voperand2);
	ex_ma.ValuResult.Write(temp_valuresult);
	ex_ma.isVwb.Write(temp_isVwb);
	//n
...
void Core::mem_access() {
	...
	unsigned long long int temp_VldResult;
	//n
	...
	unsigned long long int temp_voperand2=ex_ma.Voperand2.Read();
	unsigned long long int temp_valuresult=ex_ma.ValuResult.Read();
	bool temp_isVwb=ex_ma.isVwb.Read();
	//n
	...
	if(temp_isVld){
		if (temp_mar >= 0 && temp_mar <= MEM_CAPACITY - sizeof(unsigned int)){
			temp_valuresult = ((unsigned long long int)MEM.Read(temp_mar)<<32)|MEM.Read(temp_mar+4);
		}
	}
	else if (temp_isVst){
		//pprint(2)<<"Exceuting Instruction 0x"<<hex<<temp_instruction_word<<" with PC 0x"<<temp_PC<<endl;
		...
		//pprint(2)<<"Writing to Memory at address 0x"<<hex<<temp_aluResult<<" with data "<<dec<<temp_mdr<<endl;
		if (temp_mar >= 0 && temp_mar <= MEM_CAPACITY - sizeof(unsigned int)){
			temp_mdr=temp_voperand2>>32;
			MEM.Write(temp_mar,temp_mdr);
			temp_mdr=(unsigned int)temp_voperand2;
			MEM.Write(temp_mar+4,temp_mdr);
		}
	}
	...
	ma_rw.ValuResult.Write(temp_valuresult);
	ma_rw.isVwb.Write(temp_isVwb);
	//n
...
void Core::write_back() {
	...
	unsigned long long int temp_vresult=ma_rw.ValuResult.Read();
	bool temp_isVwb=ma_rw.isVwb.Read();
	//n
	...
	if (temp_isWb){
		...
		//pprint(2)<<"Exceuting Instruction 0x"<<hex<<temp_instruction_word<<" with PC 0x"<<temp_PC<<endl;
		if(temp_isVmov2){
			temp_result=temp_vresult>>32;
			temp_addr = inst_bitset(temp_instruction_word,23,26);
			R[temp_addr] = temp_result;
			temp_result=temp_vresult;
			temp_addr = (temp_addr+1)%14;
			R[temp_addr] = temp_result;
			...
			//cout << temp_addr<<" "<<hex<<temp_aluResult<<endl;
			R[temp_addr] = temp_result;
			fprint(1)<<"; "<<registerstring(temp_addr)<<" = 0x"<<hex<<temp_result;
		}
	...
	//n
		...
		temp_addr = inst_bitset(temp_instruction_word,23,26);
		V[temp_addr] = temp_vresult;
		//cout << "Vect ro " << hex<<temp_vresult<<endl;
		fprint(1)<<"; "<<vectorstring(temp_addr)<<" = 0x"<<hex<<temp_vresult;
	}
	//n
	else {
...
bool Core::check_data_conflict(PipelineRegister& A, PipelineRegister& B){
	...
	//////////////////////////////////////////////////////////////////n
	int inst_A=A_opcode5<<4|A_opcode4<<3|A_opcode3<<2|A_opcode2<<1|A_opcode1;
	int inst_B=B_opcode5<<4|B_opcode4<<3|B_opcode3<<2|B_opcode2<<1|B_opcode1;
	if((A_opcode5==1&&A_opcode4==0&&A_opcode3==1&&A_opcode2==0&&A_opcode1==1)&&inst_B<21){
		//A is VMOV1
		if(dest==src2||dest==(src2+1)%16){
			return true; 
		}
	}
	if((A_opcode5==1&&A_opcode4==0&&A_opcode3==1&&A_opcode2==1&&A_opcode1==0)&&inst_B>20){
		//A is VMOV2
		if(dest==src2){
			//cout<<"A"<<inst_A<<" B"<<inst_B<<endl;
			return true; 
		}
	}
	if((B_opcode5==1&&B_opcode4==0&&B_opcode3==1&&B_opcode2==1&&B_opcode1==0)&&inst_A<21){
		//B is VMOV2 and A is normal
		if(dest==src2||dest+1==src2||dest==src1||dest+1==src1){
			return true; 
		}
	}
	if((A_opcode5==1&&A_opcode4==1&&A_opcode3==1&&A_opcode2==0&&A_opcode1==1)&&inst_B<21){
		//A is vld
		if(dest==src1){
			return true; 
		}
	}
	if((A_opcode5==1&&A_opcode4==1&&A_opcode3==1&&A_opcode2==1&&A_opcode1==0)&&inst_B<21){
		//A is vst and B is normal
		if(dest==src1){
			return true; 
		}
	}
	if((A_opcode5==1&&A_opcode4==1&&A_opcode3==1&&A_opcode2==1&&A_opcode1==0)&&inst_B>20){
		//A is vst and B is vector inst
		if(dest==A_rd){
			return true; 
		}
	}
	if((inst_A<21&&inst_B<21)||(inst_A>22&&inst_A<30&&inst_B>22&&inst_B<30)){
...
string Core::disassemble (unsigned int inst_word){
	...
	//n
	if (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1){
		inst = "VMOV1" + modifier + " " + vectorstring(rd) + ", " + registerstring(rs2);
		inst += ", " + registerstring((rs2+1)%14);
	}
	if(opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0){
		inst = "VMOV2" + modifier + " " + registerstring(rd) + ", " + vectorstring(rs2);
	}
	...
	if(opcode5 == 1 && opcode4 == 1 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0){
		inst = "VST" + modifier + " " + vectorstring(rd) + ", "  + hexstring(imm) + "[" + registerstring(rs1)+ "]"; 
	}
	return inst;
